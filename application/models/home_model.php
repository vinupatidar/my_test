    ...
    class Home_Model extends CI_Model{
        ...
        public function get_story_details()
		{
			return $this->db->select("*")
                    ...
                    ->join('user','user.id_user=story.id_user')
					->where('story.delete',0)
					->order_by('story.posted_date','random')
		...
		public function get_top_story_details()
		{
			return $this->db->select("*")
					->from("story")
					->join('story_content','story_content.id_story=story.id_story')
                    //->join('user','user.id_user=story.id_user')
					->where('story.delete',0)
					->order_by('story.view_counts','desc')
					->group_by('story.id_story')
					->limit(5,0)
					->get()
					->result();
		...
		public function get_similar_stories($story_id,$category_id)
		{
			return $this->db->select("*")
					->from("story")
					->join('story_content','story_content.id_story=story.id_story')
                    ->join('user','user.id_user=story.id_user')
					->where('story.category',$category_id)
					->where('story.delete',0)
					->where('story.id_story !=',$story_id)
					->order_by('story.posted_date','random')
					->group_by('story.id_story')
					->limit(5,0)
					->get()
		...
		public function get_user_stories($user_name)
		{
			return $this->db->select("*")
					...
					->where('story.delete',0)
					->order_by('story.posted_date','desc')
					->group_by('story.id_story')
					->get()
					->result();
		}
        ...
        public function login_by_email($email,$password) {
         ...
         public function likefeed($id_story) {
            mysql_query('update story SET `story_like` = `story_like`+1 where id_story=$id_story');
        }
